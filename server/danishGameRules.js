...
danishGameRules.prototype.playCards = function (socket, cards) {
	if(this.canPlayCard(cards[0]) && this.playerManager.players[this.playerTurn].player.name == socket.player.name) //it's our turn and we can play this card
	{
		...
		if(cards[0].id == 10 || cutByFour){
			...
			if(!this.playerManager.hasCards(this.playerManager.players[this.playerTurn]))
				this.nextPlayerTurn();
		}
		else
			this.nextPlayerTurn();
...
danishGameRules.prototype.nextPlayerTurn = function () {
	...
	if(this.playerManager.playerWithCardsCount() > 1 && !this.playerManager.hasCards(this.playerManager.players[this.playerTurn]))
